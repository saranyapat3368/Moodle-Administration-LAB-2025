...
![2](https://github.com/user-attachments/assets/bae9cfc7-4b00-4344-a7d3-1d79bf9d67cd)
...
![3](https://github.com/user-attachments/assets/7b93bbc7-193d-48a9-8f1e-9cc8595e8a13)
...
![4](https://github.com/user-attachments/assets/1321f5d3-43a6-478f-beab-c98ed743ebe9)
...
**บันทึกผลการทดลอง 5:**
![5](https://github.com/user-attachments/assets/197a15bc-d8b9-4694-bbaf-3e99a69f9b04)
...
**บันทึกผลการทดลอง 6:**
![6](https://github.com/user-attachments/assets/9099e20b-5469-4136-b5cc-7a23a375b3e5)
...
![7](https://github.com/user-attachments/assets/d44197cd-16b6-4d48-b8b0-b1133e926d40)
...
**บันทึกผลการทดลอง 7:**
![7](https://github.com/user-attachments/assets/37d5d4f5-0a6d-4a6e-b280-89d58733b4eb)
...
**บันทึกผลการทดลอง 8:**
![8](https://github.com/user-attachments/assets/92ccccb3-fb47-48ec-aa83-ebf7468f178f)
...
**บันทึกผลการทดลอง 9:**
![9](https://github.com/user-attachments/assets/f948e3e6-b8e6-4f51-9182-46c7e805503b)
...
**บันทึกผลการทดลอง 10:**
![10](https://github.com/user-attachments/assets/847df2b6-c3db-47ec-a32c-cd5feafefad7)
...
5. 
   **บันทึกผลการทดลอง 11:**
![11](https://github.com/user-attachments/assets/46c42510-d33a-4845-912d-3fcea2f9814f)
...
คำตอบ: Docker Image = ไฟล์ต้นแบบ (Template) ที่ใช้สำหรับสร้าง Container ภายใน Image จะประกอบด้วยระบบปฏิบัติการย่อย ไลบรารี และโปรแกรมที่จำเป็นทั้งหมด เปรียบเสมือน “แม่พิมพ์” หรือ “ไฟล์ติดตั้ง” ที่ยังไม่ถูกนำไปใช้งานจริง
Docker Container = Instance ที่ถูกสร้างขึ้นจาก Image และกำลังทำงานอยู่ เปรียบเสมือน โปรแกรมที่ถูกติดตั้งและเปิดใช้งานแล้ว

ตัวอย่างเช่น Image moodle:latest เป็นไฟล์ต้นแบบของ Moodle เมื่อรัน Image นี้จะได้ Container moodle_app ซึ่งเป็น Moodle ที่สามารถเข้าใช้งานผ่านเว็บได้จริง
```
...
คำตอบ: 1 moodle_db ทำหน้าที่เป็น Database Server ของระบบ Moodle โดยมีหน้าที่จัดเก็บข้อมูลทั้งหมดที่สำคัญ เช่น ข้อมูลผู้ใช้ ข้อมูลรายวิชา คะแนน การตั้งค่าระบบ และข้อมูลการใช้งานต่าง ๆ ของ Moodle ฐานข้อมูลนี้ถูกเก็บแบบถาวรผ่าน Named Volume db_data เพื่อให้ข้อมูลไม่สูญหายแม้จะมีการหยุดหรือลบ Container และ Container นี้จะทำงานอยู่ภายในเครือข่าย moodle_network เพื่อรอรับการเชื่อมต่อจาก Moodle Application

2 moodle_app ซึ่งสร้างจาก Image lthub/moodle:education-4.5.8 ทำหน้าที่เป็น Web Application Server สำหรับให้ผู้ใช้งานเข้าใช้งานระบบ Moodle ผ่านเว็บเบราว์เซอร์ โดย Container นี้จะเชื่อมต่อไปยังฐานข้อมูลที่อยู่ใน moodle_db ผ่าน Docker Network โดยใช้ชื่อ Service db เป็น Database Host นอกจากนี้ยังมีการเปิดพอร์ต 80:80 เพื่อให้สามารถเข้าใช้งาน Moodle ผ่าน http://localhost และมีการใช้ Volume moodledata เพื่อเก็บไฟล์ข้อมูลของ Moodle เช่น ไฟล์รายวิชาและไฟล์ที่ผู้ใช้อัปโหลด
```
...
คำตอบ:named volume 
ข้อดี ข้อมูลจะไม่หายแม้ Container ถูกลบหรือสร้างใหม่ ทำให้เหมาะกับระบบจริง
ข้อเสีย หากสั่งลบ Volume โดยตรง (เช่น docker-compose down -v) ข้อมูลทั้งหมดจะสูญหาย และการย้ายข้อมูลออกไปใช้งานภายนอกอาจทำได้ยากกว่าการใช้ bind mount
```
...
คำตอบ: ทำหน้าที่เป็นเครือข่ายเสมือนที่เชื่อม Container ต่าง ๆ เข้าด้วยกัน ทำให้ Container สามารถติดต่อสื่อสารกันได้โดยใช้ชื่อ Service เป็น hostname
```
...
คำตอบ: ควบคุมลำดับการทำงานของ Container ทำให้ระบบที่มีความสัมพันธ์กันอย่าง Moodle และ Database สามารถเริ่มต้นได้อย่างถูกต้องและลดความผิดพลาดในการเชื่อมต่อระหว่าง Service
```
...
คำตอบ: ต้องแก้ไขที่ส่วน ports ของ service moodle ในไฟล์ docker-compose.yml 
จาก ports:   - "80:80" เปลี่ยนเป็น  "9000:80"
```
...
คำตอบ: MOODLE_DB_HOST=db คือการกำหนดให้ Moodle เชื่อมต่อฐานข้อมูลผ่านชื่อ service db ใน Docker network เดียวกัน ไม่ใช้ localhost เพราะ localhost ภายใน container หมายถึงตัวเอง ไม่ใช่ container ฐานข้อมูล
```
...
คำตอบ: ข้อดี - ติดตั้งได้รวดเร็ว ใช้คำสั่งไม่กี่บรรทัด ไม่ต้องตั้งค่า Web Server, PHP และ Database แยกทีละขั้น
- แยกส่วนการทำงานชัดเจน เช่น Web (Moodle) และ Database อยู่คนละ Container
- ย้ายเครื่องหรือ Deploy ซ้ำได้ง่าย เพียงมีไฟล์ docker-compose.yml
- ลบหรือสร้างใหม่ได้สะดวก โดยไม่กระทบระบบหลักของเครื่อง

ข้อเสีย
- ต้องมีความรู้ Docker และ Docker Compose เพิ่มเติม
- ใช้ทรัพยากรมากกว่าการติดตั้งตรงเล็กน้อย
- การ Debug บางกรณีซับซ้อนกว่าการดูไฟล์บนเครื่องโดยตรง
- ผู้เริ่มต้นอาจสับสนเรื่อง Network, Volume และ Environment Variables
```
...
```
วิธีตรวจสอบ:
- ตรวจสอบว่า Container ฐานข้อมูล (moodle_db) ทำงานอยู่หรือไม่ ด้วยคำสั่ง docker ps
- ตรวจสอบ Log ของ Container moodle_app ด้วยคำสั่ง docker logs moodle_app เพื่อดูข้อความ error การเชื่อมต่อฐานข้อมูล
- ตรวจสอบค่า Environment Variables ที่เกี่ยวข้องกับ Database เช่น MOODLE_DB_HOST, MOODLE_DB_NAME, MOODLE_DB_USER และ MOODLE_DB_PASSWORD ว่าตรงกับค่าที่ตั้งไว้ใน Container db หรือไม่
- ตรวจสอบว่า Container moodle_app และ db อยู่ใน Docker network เดียวกันหรือไม่
- ตรวจสอบชื่อ Service/Container ของ Database ว่าตรงกับค่าที่กำหนดใน MOODLE_DB_HOST หรือไม่


วิธีแก้ไข:
- แก้ไขค่า Environment Variables ให้ถูกต้องและตรงกันระหว่าง moodle_app และ db
- หากฐานข้อมูลยังไม่พร้อมใช้งาน ให้รีสตาร์ท Container ด้วยคำสั่ง docker-compose restart หรือ docker-compose up -d
- ตรวจสอบและแก้ไขชื่อ Database Host ให้ใช้ชื่อ service db แทนการใช้ localhost
- หากยังไม่สามารถเชื่อมต่อได้ ให้ลบและสร้าง Container ใหม่โดยไม่ลบ Volume ด้วยคำสั่ง docker-compose down และ docker-compose up -d
```

**11. ถ้ารัน `docker-compose down -v` จะเกิดอะไรขึ้นกับข้อมูล?**

คำตอบ: - docker-compose down → ลบ container แต่ ข้อมูลยังอยู่
- docker-compose down -v → ลบ container และข้อมูลทั้งหมด
```
